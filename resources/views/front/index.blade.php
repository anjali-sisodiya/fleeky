      ...
      <div id="sequence" class="seq">
        <div class="seq-screen">
          <ul class="seq-canvas">
            ...
            <li>
              <div class="seq-model">
                <!-- <img data-seq src="{{asset($list->image)}}" /> -->
                <img src="{{ asset($list->image) }}" alt="Image" />

              </div>
              @if($list->btn_txt!='')
              <div class="seq-title">
                 <!-- <h2 data-seq>{{$list->desc}}</h2>  -->
                <!-- <p data-seq>{{$list->short_desc}}</p> -->
                <a data-seq target="_blank" href="{{$list->btn_link}}" class="aa-shop-now-btn aa-secondary-btn" style="margin-top:91px;padding:20px;width:180px;
                font-weight:bold;font-size:20px;">{{$list->btn_txt}}</a>
              </div>
              @endif
            </li>     
            @endforeach
    ...
    <div class="row">
      <div class="col-md-4 offset-md-4">
        <div class="aa-promo-area">
          ...
          <div class="row" style="position: relative;left: 106%;">
            <div class="col-md-12 no-padding">
              <div class="aa-promo-right">
                @foreach($home_categories as $list)
                <div class="aa-single-promo-right">
                  <div class="aa-promo-banner" style="padding: 9px;">                      
                    <img src="{{asset($list->category_image)}}" style="height: 260px" alt="img">                      
                    <div class="aa-prom-content">
                      <h4><a href="{{url('category/'.$list->category_slug)}}" class="text-dark">{{$list->category_name}}</a></h4>                        
                    </div>
                  </div>
                </div>
                @endforeach
                    ...
                    <div class="tab-pane fade {{$cat_class}}" id="cat{{$list->id}}">
                      ...
                      @if(isset($home_categories_product[$list->id][0]))
                       @foreach($home_categories_product[$list->id] as $productArr)
                        <li>
                          <figure>
                            <a class="aa-product-img" href="{{url('product/'.$productArr->slug)}}"><img src="{{asset($productArr->image)}}" style="width: 196px; margin-left: 55px;" alt="{{$productArr->name}}"></a>
  ...
  <section id="aa-banner">
    <div class="container">
      <div class="row">
        <div class="col-md-12">        
          <div class="row">
            <div class="aa-banner-area" style="margin-top: 20px;">
            <a href="#"><img src="{{asset('front_assets/img/home-banner1.jpg')}}" alt="fashion banner img" style="height: 319px;width: 100%;"></a>
          </div>
          </div>
        </div>
      </div>
                  ...
                  <ul class="aa-product-catg aa-featured-slider">
                    ...
                    @if(isset($home_featured_product[$list->id][0]))
                       @foreach($home_featured_product[$list->id] as $productArr)
                        <li>
                          <figure>
                            <a class="aa-product-img" href="{{url('product/'.$productArr->slug)}}"><img src="{{asset($productArr->image)}}" style="width: 196px;" alt="{{$productArr->name}}"></a>
                            <a class="aa-add-card-btn" href="javascript:void(0)" onclick="home_add_to_cart('{{$productArr->id}}','{{$home_product_attr[$productArr->id][0]->size}}','{{$home_product_attr[$productArr->id][0]->color}}')"><span class="fa fa-shopping-cart"></span>Add To Cart</a>
                            <figcaption>
                              <h4 class="aa-product-title"><a href="{{url('product/'.$productArr->slug)}}">{{$productArr->name}}</a></h4>
        ...
        <div class="col-md-12">
          <div class="aa-client-brand-area">
            <ul class="aa-client-brand-slider">
              ...
              <li><a href="#"><img src="{{asset($list->image)}}" alt="{{$list->name}}"></a></li>
              @endforeach
            </ul>
          </div>
        </div>
