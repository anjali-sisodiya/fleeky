...
@endif
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
               <button class="btn btn-success"><a href="/add_home_banner" class="text-decoration-none text-white">Add Banner</a></button>
    ...
    @foreach($show as $member)
                    ...
                    <tr>
                      <td>{{ $member->id }}</td>
                      <td>{{ $member->btn_txt }}</td>
                      <td>{{ $member->btn_link }}</td>
                      <td>
                          <img src="{{ asset($member->image) }}" alt="Banner Image" width="75">
                      </td>
                      <td>
    @if($member->status == 1)
        <form method="POST" action="{{ route('home_banner_status', ['status' => 0, 'id' => $member->id]) }}">
            @csrf
            @method('PUT')
            <button type="submit" class="btn ms-1 reduced-font btn-success btn-sm text-white">Active</button>
        </form>
    @elseif($member->status == 0)
        <form method="POST" action="{{ route('home_banner_status', ['status' => 1, 'id' => $member->id]) }}">
            @csrf
            @method('PUT')
    ...
    @endif
                      ...
                      <td> 
                          <a class="text-decoration-none text-white" href="edit_home_banner/{{ $member->id }}">
                                <button type="button" class="btn reduced-font btn-primary btn-sm text-white">Edit
                                        <i class="fa-solid fa-pen-to-square reduced-font ms-1"></i>
                                </button>
                          </a>

                          <a class="text-decoration-none text-white" href="delete_home_banner/{{ $member->id }}">
