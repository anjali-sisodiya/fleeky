...
<div class="content-wrapper">
  ...
  {{ session('success') }}
  <button type="button" class="btn-close mt-2" data-bs-dismiss="alert" aria-label="Close" style="float: right;"></button>
</div>
@endif
   @if(session('error'))
            ...
            <div class="col-12">
               <div class="card">
                  <div class="card-header">
                     <button class="btn btn-success"><a href="/add_brand" class="text-decoration-none text-white">Add brand</a></button>
                     ...
                     <table class="table table-hover text-nowrap table-bordered text-center">
                        ...
                        <tbody>
                           ...
                           <tr>
                              ...
                              <td>{{ $member->name }}</td>
                              <td>
                                 <img src="{{ asset($member->image) }}" alt="Brand Image" width="75">
                              </td>
                              <td>
                                 @if($member->status == 1)
                                 <form method="POST" action="{{ route('brand_status', ['status' => 0, 'id' => $member->id]) }}">
                                    @csrf
                                    @method('PUT')
                                    <button type="submit" class="btn ms-1 reduced-font btn-success btn-sm text-white">Active</button>
                                 </form>
                                 @elseif($member->status == 0)
                                 <form method="POST" action="{{ route('brand_status', ['status' => 1, 'id' => $member->id]) }}">
                                    @csrf
                                    @method('PUT')
                              ...
                              <td> 
                                 <a class="text-decoration-none text-white" href="edit_brand/{{ $member->id }}">
                                 ...
                                 <a class="text-decoration-none text-white" href="delete_brand/{{ $member->id }}">
